...
namespace Nop.Plugin.Shipping.Speedy
{
    public partial class OrdersConsumer : IConsumer<OrderPlacedEvent>
    {
        ...
        public OrdersConsumer(IShipmentContext shipmentContext, ILogger logger, IRepository<ShipmentTask> shipmentTasksRep, IGenericAttributeService genericAttributeService, IWorkContext workContext)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(order.ShippingRateComputationMethodSystemName) && order.ShippingRateComputationMethodSystemName.Equals("Shipping.Speedy"))
                {
                    ...
                    if (bol != null)
                    {
                        ...
                        {
                            var shipmentData = new ShipmentTask
                            {
                                SiteId = bol.SiteId,
                            ...
                            _shipmentTasksRep.Insert(shipmentData);
