...
namespace Nop.Plugin.Shipping.Speedy.Services
{
    public class SpeedyShipmentsService : ISpeedyShipmentsService
    {
        ...
        public void RunCheckingShipmentsList()
        {
            ...
            var notCreatedBolItems = _speedyShipmentRep.Table.Where(w => w.BolCreatingStatus == BolCreatingStatus.Pending).ToList();
        ...
        public void CancelBol(int shipmentId, string message)
        {
            try
            {
                using (var srv = GetSpeedyClient())
                {
                    ...
                    var shipment = _speedyShipmentRep.GetById(shipmentId);
                    if (shipment != null)
                    {
        ...
        private void CreateBol(SpeedyShipment shipment)
        {
            using (var srv = GetSpeedyClient())
            {
        ...
        {
            var shipments = _speedyShipmentRep.Table.Where(w =>
